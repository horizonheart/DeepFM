...
class DeepFM(BaseEstimator, TransformerMixin):
    def __init__(self, feature_size, field_size,
        ...
        self.feature_size = feature_size # feature_size是原始特征进行one-hot编码后的总的特征长度      # denote as M, size of the feature dictionary
        self.field_size = field_size #字段的个数            # denote as F, size of the feature fields
    ...
    def fit(self, Xi_train, Xv_train, y_train,
        ...
        for epoch in range(self.epoch):
            t1 = time()
            self.shuffle_in_unison_scary(Xi_train, Xv_train, y_train)#打乱顺序
            total_batch = int(len(y_train) / self.batch_size)
            for i in range(total_batch):
                Xi_batch, Xv_batch, y_batch = self.get_batch(Xi_train, Xv_train, y_train, self.batch_size, i)#todo 获取一个批次的数据
                # todo 训练一个批次的数据
